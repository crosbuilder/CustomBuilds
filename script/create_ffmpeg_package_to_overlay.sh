...
tar cvf - libffmpeg-free | (cd ~/trunk/src/overlays/overlay-${BOARD}/chromeos-base; tar xf -)
if [ 0 -ne $? ]; then
...
# ebuildのテストを実行する
cd ~/trunk/src/overlays/overlay-${BOARD}/chromeos-base/libffmpeg-free
rm -f Manifest
...
# 依存関係を追加
cd ~/trunk/src/overlays/overlay-${BOARD}/virtual/target-chromium-os
