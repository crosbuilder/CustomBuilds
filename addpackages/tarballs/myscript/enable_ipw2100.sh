...
if [ ! -e /var/lib/preload-network-drivers ]; then
  ...
  echo ipw2100 > /var/lib/preload-network-drivers
  echo preload-network-drivers created.
else
  g=`grep ipw2100 /var/lib/preload-network-drivers`
  if [ -z "$g" ]; then
    echo preload-network-drivers not contains ipw2100.
    sed -e '1i ipw2100' -i /var/lib/preload-network-drivers
    echo add entry ipw2100 to preload-network-drivers
  else
