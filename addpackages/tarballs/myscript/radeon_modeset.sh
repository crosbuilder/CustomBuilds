...
for cfgdir in ${cfgdirlist[@]}; do
  ...
  for cfg in ${cfglist[@]}; do
    ...
    content_new=`echo "${content}" | sed -e "s/i915.modeset=1/i915.modeset=0/g" -e "s/radeon.modeset=0/radeon.modeset=1/g" -e "s/nouveau.modeset=1//g"`
    if [ "${content}" != "${content_new}" ]; then
      echo modify ${cfgdir}/syslinux/${cfg} | tee /dev/tty1 | logger -t myscript
